...
ALuint Buffer[MUSICNUM];
ALuint Source[MUSICNUM];
...
ALboolean LoadWave(){
	wavelist("WAVE/stratmenu.wav",		  MUSIC_START,		0.5f);
	wavelist("WAVE/ourturn.wav",		  MUSIC_OUR,		0.5f);
	wavelist("WAVE/enemyturn.wav",		  MUSIC_ENEMY,		0.5f);
	wavelist("WAVE/battle.wav",			  MUSIC_BATTLE,		0.5f);
	wavelist("WAVE/battle_boss.wav",	  MUSIC_BOSS,		0.5f);
	wavelist("WAVE/lvup.wav",			  MUSIC_LVUP,		0.5f);
	wavelist("WAVE/gameover.wav",		  MUSIC_GAMEOVER,	0.5f);
	wavelist("WAVE/allclear.wav",		  MUSIC_ALLCLEAR,	0.5f);
...
ALboolean wavelist(char *title, int num, float volume){
	...
	alSourcei (Source[num], AL_LOOPING,  AL_TRUE	);			//loop control
